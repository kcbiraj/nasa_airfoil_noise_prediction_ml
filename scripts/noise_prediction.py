...
df = spark.read.csv(
    "../datasets/Raw/NASA_airfoil_noise_raw.csv", header=True, inferSchema=True
)
...
df.write.mode("overwrite").parquet(
    "../datasets/Cleaned/NASA_airfoil_noise_cleaned.parquet"
)
...
print(
    "Total rows after dropping dupicate rows and null values from the dataset =",
    rowcount3,
)

print("Renamed column name =", df.columns[5])
print(
    "NASA_airfoil_noise_cleaned.parquet exists:",
    os.path.isdir("NASA_airfoil_noise_cleaned.parquet"),
)
...
df = spark.read.parquet("../datasets/Cleaned/NASA_airfoil_noise_cleaned.parquet")
...
evaluator = RegressionEvaluator(
    predictionCol="prediction", labelCol="SoundLevelDecibels", metricName="mse"
)
...
evaluator = RegressionEvaluator(
    predictionCol="prediction", labelCol="SoundLevelDecibels", metricName="mae"
)
...
evaluator = RegressionEvaluator(
    predictionCol="prediction", labelCol="SoundLevelDecibels", metricName="r2"
)
